AquaAyur 🌿📱
Expo v56.0.0 React Native v0.85 TypeScript Supabase Groq AI
...
Set up a project on your Supabase Dashboard.
Open the SQL Editor in the Supabase Dashboard.
Copy and run the SQL instructions in supabase_complete_schema.sql to provision all tables, columns, indexes, and Row Level Security (RLS) policies.
(Optional Dev Step) If you need to disable RLS restrictions during local development to simplify testing, copy and run the SQL commands in disable_rls_dev.sql.
🔐 Step 4: Authentication Setup (Clerk)
Set up a project on your Clerk Dashboard.
Navigate to JWT Templates in the Clerk sidebar.
Click New Template and select Supabase.
Keep the template name as supabase to ensure compatibility with the token exchange logic.
Configure sign-in methods (Email/Password, Google OAuth, etc.) as preferred.
...
npm run start
Scan the displayed QR code with the Expo Go app on your phone (iOS / Android).
